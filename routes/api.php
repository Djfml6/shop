...
Route::namespace('Api')->group(function(){
    ...
    // 获取站点协议
    Route::get('agreement/{agreements:ename}','AgreementController@show'); 
    ...
    // 商品
    Route::get('goods/{id}','GoodsController@show'); // 获取商品详情
    ...
    Route::get('store/{id}','StoreController@show'); // 店铺首页
    Route::get('store/{id}/goods','StoreController@get_store_goods_list'); // 店铺首页商品列表

    // 积分商城
    Route::get('integrals','IntegralController@index'); // 首页数据
    Route::get('integral/goods/{id}','IntegralController@show')->where('id', '[1-9]+'); // 积分商品详情
    ...
    Route::group(['middleware'=>'jwt.user'],function(){
        ...
        // 用户资料
        Route::get('users/info','UserController@info'); // 获取用户资料
        ...
        // 订单处理
        Route::post('order/create_order_before','OrderController@create_order_before'); // 生成订单前处理
