...
class StoreService extends BaseService{
    ...
    public function getHomeStoreList(){
        ...
        $store_model = $store_model->where('store_status', true)->select(DB::raw('*,'.$distance))->withCount(['comments','comments as good_comment' => function($q){
            $q->whereRaw('(score+agree+speed+service)>=15');
        }]);

        $list = $store_model->orderBy('distance','desc')->paginate(request()->per_page ?? 30);
        return $list;
    ...
    public function getStoreInfo($store_id, $select = ""){ 
        ...
        if(empty($store_info)){
            ...
            throw new RequestException(CodeResponse::STORE_INVALID);
    ...
    public function getStoreInfoAndRate($store_id, $select = ""){
        $store_info = $this->getStoreInfo($store_id,$select);
        $info = OrderComment::query()->where('store_id',$store_id)->first([
