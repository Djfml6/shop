...
class MoneyLogService extends BaseService{
     ...
     */
    public function editMoney($name, $user, $money, $type = Constant::MONEY_TYPE_BALANCE, $info=''){
        try{
            DB::beginTransaction();
            $user->money()->create([
                'money' => $money,
            ...
            switch($type){
                case Constant::MONEY_TYPE_BALANCE:
                    $user->money += $money;
                break;
                case Constant::MONEY_TYPE_FROZEN:
                    $user->frozen_money += $money;
                break;
                case Constant::MONEY_TYPE_INTEGRAL:
                    $user->integral += $money;
                break;
            }
            $user->save();
